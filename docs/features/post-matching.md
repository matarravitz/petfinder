...
**Scoring (client-side, over the prefiltered candidate set):** a weighted blend of signals, each normalized to roughly 0-1:
...
- **Fields (breed/color/size) match** (weight 0.5 in the no-embedding case, 0 otherwise): added 2026-07-16 after a real false-positive ("Rex," a brown Labrador mix, scored a "Strong match" against an unrelated white-and-brown Terrier mix purely on location+date). Compares breed/color/size case-insensitively, averaging only the fields **both** posts actually have set (a post with an unknown breed doesn't count as a mismatch) — `null` (not `0`) when nothing is comparable at all, so the caller can fall back rather than treating "unknown" as "different."

If either post in a pair has `photo_embedding: null` (true for every post that didn't go through the live `CreatePostForm` embedding flow — this includes every seeded demo post), drop the visual-similarity term entirely. Then: if fields data is comparable between the two posts, **renormalize to fields 0.5 / location 0.3 / date 0.2** (fields dominates, since it's the only concrete identifying signal left); if fields aren't comparable either, fall back further to **location 0.6 / date 0.4** (the original, pre-2026-07-16 fallback) rather than scoring it as 0 or excluding the candidate — an old post without any comparable data should still be able to surface as a location/date-only match, just a weaker one.

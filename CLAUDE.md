...
- **The "Mark as resolved" flow is a friendly `.resolve-prompt` card (`PostDetailPage.jsx`), not a plain button** — deliberately reworked from a bare `<button>Mark as resolved</button>` after user feedback that a plain resolve action reads as presumptuous ("did you find your pet?" can feel bad to answer when the search didn't end happily). Copy is type-aware (`missing` vs `found`) via `isMissing` ternaries in both the card and the `resolve-prompt-remove-link` button beneath it. That second, deliberately muted (not hidden) link triggers `deletePost` (via `window.confirm`, this app's standard destructive-action pattern) as a neutral, equally-legitimate alternative to a happy resolution — don't reintroduce a single-button "resolve or nothing" pattern here.
